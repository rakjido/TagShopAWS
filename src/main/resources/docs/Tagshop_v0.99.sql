...
-- -----------------------------------------------------
-- Table `tagshop`.`Settlement`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `tagshop`.`Settlement` ;

CREATE TABLE IF NOT EXISTS `tagshop`.`Settlement` (
  `BasketId` BIGINT NOT NULL,
  `ImpUid` VARCHAR(50) NULL,
  `MerchantUid` VARCHAR(50) NULL,
  `PaidAmount` VARCHAR(50) NULL,
  `ApplyNum` VARCHAR(50) NULL,
  INDEX `fk_Settlement_Baskets1_idx` (`BasketId` ASC),
  PRIMARY KEY (`BasketId`),
  CONSTRAINT `fk_Settlement_Baskets1`
    FOREIGN KEY (`BasketId`)
    REFERENCES `tagshop`.`Baskets` (`BasketId`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
