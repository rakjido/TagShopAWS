...
public class UserMailSendService {
	...
	private SqlSession sqlsession;
		...
		public void mailSendWithUserKey(String e_mail, String user_id, HttpServletRequest request) {
			...
			String key = RandomValueUtil.getKey(false, 20);
			...
			String htmlStr = "<h2>안녕하세요 TAG # 입니다!</h2><br><br>" 
					...
					+ "<a href="+ request.getRequestURL().toString().replace(request.getRequestURI(),"") + request.getContextPath() + "/users/key_alter?user_id="+ user_id +"&user_key="+key+">인증하기</a></p>"
					+ "(혹시 잘못 전달된 메일이라면 이 이메일을 무시하셔도 됩니다)";


			try {
				mail.setSubject("[본인인증] TAG# 가입 인증메일입니다", "utf-8");
